...
Pages.configuracoes = function (container) {
  const render = () => {
      ...
      <section class="card">
        ...
        <form id="profile-form" class="form">
          <label class="form__field"><span>Nome</span>
            <input class="input" id="profile-name" type="text" value="${Auth.currentUser().name}" />
          </label>
          <label class="form__field"><span>E-mail</span>
            <input class="input" id="profile-email" type="email" value="${Auth.currentUser().email}" />
          </label>
    ...
    container.querySelector("#profile-form").addEventListener("submit", (e) => {
      ...
      const result = Auth.updateProfile(Auth.currentUser().id, { name, email });
      if (Components.showFormError(form, result.success ? null : result.error)) return;
      AppShell.refreshUser();
      render(); // reflete o novo nome/e-mail nos campos
    });
