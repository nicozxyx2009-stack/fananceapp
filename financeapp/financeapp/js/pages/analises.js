  ...
  const openRecurringForm = (editId = null) => {
    ...
    Components.openModal(`
      ...
      <form id="rec-form" class="form">
        ...
        <label class="checkbox-field"><input type="checkbox" id="rec-active" ${!existing || existing.active !== false ? "checked" : ""} /><span>Ativo (entra na previsão e nos totais)</span></label>
                ...
                ? State.data.recurring
                    .map((r) => {
                      ...
                      return `<tr style="${inactive ? "opacity:0.5" : ""}">
                        <td>${r.name}${inactive ? ' <span class="tag">Inativo</span>' : ""}</td>
                        <td><span class="tag">${r.category}</span></td>
                        <td>${account ? account.bank : "—"}</td>
                        <td>${inactive ? "—" : Utils.fullDate(Calculations.getNextBillingDate(r))}</td>
